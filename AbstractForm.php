...
abstract class AbstractForm
{
	 ...
	 * Отрисовщик формы
	 *
	 * @var \Korus\Icecat\Admin\Form\Renderer;
	 */
	protected $renderer;
			...
			foreach ($items as $tab) {
				...
				foreach ($controls as $name => $control) {
					if ($control['TYPE'] == 'TABS') {
						$items = is_callable($control['ITEMS']) ? call_user_func($control['ITEMS'], [])
							: $control['ITEMS'];
	...
	public function processRequest()
	{
		...
		foreach ($this->getFieldNames() as $fieldName) {
			if (isset($data[$fieldName]) && isset($this->editItem[$fieldName])) {
				if (is_object($this->editItem) && method_exists($this->editItem, '__set')) {
					$this->editItem->__set($fieldName, $data[$fieldName]);
				}
				else {
		...
		foreach ($aTabs as $aTab) {
			...
			foreach ($controls as $controlName => $controlData) {
				...
				if ($controlData['TYPE'] == 'TABS') {
					$items = is_callable($controlData['ITEMS']) ? call_user_func($controlData['ITEMS'], [])
						: $controlData['ITEMS'];
